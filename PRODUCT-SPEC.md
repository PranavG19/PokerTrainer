...
**AMENDMENT (v2.1, single-player STANDING).** Two originally-listed non-goals are consciously revised here rather than silently overwritten — the same convention "multiplayer" followed before it was built. (1) **Local multiplayer** shipped (opt-in local relay, single-player remains the default); real-money/accounts/cloud stay out. (2) A single-player **STANDING** ("Depth") is now IN scope, and the "no promise expressed as … rank" non-goal is narrowed, NOT removed. The reasoning behind the original ban still holds and is preserved: a rank that rewards *showing up* or *chip luck* would corrupt the teaching mission and reinstall the variance signal the method deletes. The amendment is scoped precisely so it cannot:
- **What it is.** Standing is the **table depth (effective big blinds) the learner has earned** — a skill estimate computed ONLY from decision quality (`coach.gradeDecision` EV-loss on sound-math charges) plus mastery/puzzle gates, and NEVER from chips. A correct all-in that busts grades ~0 EV-loss, so a bust cannot move it. It ratchets (no demotion event) and reads a thin sample pessimistically (CI-upper bound), so neither a heater nor a bad night moves standing. See `src/core/standing.ts`.
- **What stays banned, unchanged.** `BANNED_PHRASINGS` (progress.ts) and its enforcement are UNTOUCHED — the words "rank/percentile/level/streak/XP/leaderboard/badge/trophy" remain forbidden in every string the progress and coach surfaces emit. Standing is named by the concrete table depth ("40bb table" … "200bb table", "Calibrating"), never by any banned word, and is verified clean against the ban list in `standing.test.ts`. There is **no leaderboard, no percentile, no comparison to other players** (single-player is per-local-profile; multiplayer is explicitly UNRANKED). The reward for climbing is a **deeper, harder game** (higher SPR) and a **quieter coach**, never chips-for-power, cosmetics, or a shop.
- **Surface scope.** Standing appears on the **Play/Profile surface only**. The Progress and coach teaching surfaces keep obeying the full ban. The tier constants remain provisional pending a spec-literal audit against real decision logs.
